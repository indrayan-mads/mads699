#!/usr/bin/env python3
"""
build_ai_jobs_dataset.py
========================
Builds a single analysis-ready dataset linking:

  1. BLS Employment Projections from a PRE-generative-AI vintage (default 2021-31,
     finalized Sept 2022, i.e. before ChatGPT) and a POST-AI vintage (default
     2024-34, released Aug 28, 2025, which explicitly incorporates AI research).
  2. Occupation-level AI exposure measures:
       - Felten/Raj/Seamans AIOE (Strategic Mgmt Journal 2021)
       - Eloundou et al. "GPTs are GPTs" LLM exposure (OpenAI/Science 2024)
       - Anthropic Economic Index job exposure (real Claude usage), optional
  3. Task-level data:
       - O*NET task statements + importance ratings
       - Eloundou et al. task-level exposure labels (optional)
       - Anthropic Economic Index task usage / penetration (optional)

Outputs (written to ./out):
  occupation_ai_panel.csv   -> one row per 2018 SOC occupation (analysis table)
  occupation_task_long.csv  -> one row per occupation x O*NET task, with the
                               occupation-level columns broadcast onto every row
                               (the "everything in one file" version)

Usage:
  pip install pandas openpyxl requests
  python build_ai_jobs_dataset.py
  python build_ai_jobs_dataset.py --pre 2019-29 --post 2024-34
  python build_ai_jobs_dataset.py --skip-aei          # skip Anthropic Economic Index
  python build_ai_jobs_dataset.py --skip-optional     # BLS + O*NET + Felten + Eloundou occ only

Notes:
  - Downloads are cached in ./raw. If a URL breaks, download the file manually
    into ./raw using the printed instructions and re-run; the script uses the
    cached copy.
  - Only the two BLS vintages and O*NET tasks are REQUIRED; every exposure
    source is merged best-effort and skipped with a warning if unavailable.
"""
